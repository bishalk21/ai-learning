...
function App() {
  ...
  const sendMessage = async () => {
    try {
      ...
      setMessages((prev) => [...prev, data.translatedText]);
  ...
  return (
    <div className="App space-y-4 p-4 flex flex-col items-center gap-4 h-screen relative">
      {/* Message container */}
      <div className="w-full max-w-md max-sm:p-4 flex flex-col gap-2">
        {messages.map((message, index) => (
          <div key={index} className="bg-gray-200 text-gray-800 p-2 rounded">
            {message}
          </div>
        ))}
      </div>
      ...
      {/* Message input area */}
      <div className="w-full max-w-md max-sm:p-4 flex gap-2 items-center justify-center absolute bottom-4">
        <textarea
          id="messageInput"
          rows={1}
